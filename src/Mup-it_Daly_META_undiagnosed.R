...
# for each gene of different functional classes, then estimate the probability of
# seeing the observed combination of different functional classes of DNMs 
...
our.data = read.delim(file.path(DE_NOVO_DIR, "DNG_Variants_20Feb2014_NonRed_Clean_NoTwins_NoClusters.txt"), header=T)
# remove diagnosed patients, if maximising power
...
########### Epi4K dataset ############
epi4k = read.delim(file.path(DE_NOVO_DIR, "epi4k_v2.txt"), header=T)
...
########### Autism dataset ###############
autism = read.delim(file.path(DE_NOVO_DIR, "autism_v3_PJ.txt"), header=T)
...
fromer = read.delim(file.path(DE_NOVO_DIR, "fromer_v2.txt"), header=T)
...
zaidi = read.delim(file.path(DE_NOVO_DIR, "zaidi_VEP.txt"), header=T)
