...
#' @param ddd_clust path to clustering results for the ddd only subset
#' @param ddd_enrich path to enrichment results for the ddd only subset
#' 
...
#'     each gene.
get_most_significant <- function(meta_clust, meta_enrich, ddd_clust, ddd_enrich) {
    ...
    ddd = combine_analyses(ddd_enrich, ddd_clust) 
...
merged = get_most_significant(meta_clust, meta_enrich, ddd_clust, ddd_enrich)
write.table(merged, file=file.path(RESULTS_DIR, "de_novos.ddd_4k.all.txt"), row.names=F, quote=F, sep="\t")


merged = get_most_significant(meta_clust, seizures_meta_enrich, ddd_clust, seizures_enrich)
