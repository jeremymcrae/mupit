...
open_ddd_de_novos <- function(diagnosed, sample_ids=NULL) {
    ...
    variants = read.table(file.path("data-raw", "de_novo_datasets", "DNG_Variants_20Feb2014_NonRed_Clean_NoTwins_NoClusters.txt"), header=TRUE, sep="\t", stringsAsFactors=FALSE, comment.char="")
    ...
    variants$hgnc = variants$curated_HGNC
    variants$consequence = variants$curated_CQ
    
    vep = apply(variants, 1, get_vep_consequence, verbose=TRUE)
    
    variants$new_consequence = sapply(vep, "[", 1)
    variants$new_hgnc = sapply(vep, "[", 2)
    
    variants$gene_match = variants$hgnc == variants$new_hgnc
    variants$consequence_match = variants$consequence == variants$new_consequence
    
    variants[!variants$gene_match, c("chrom", "start_pos", "ref_allele", "alt_allele", "hgnc", "new_hgnc")]
    variants[!variants$consequence_match, c("chrom", "start_pos", "ref_allele", "alt_allele", "hgnc", "new_hgnc", "consequence", "new_consequence")]
    
    variants$study_code = "ddd_nature_2014"
