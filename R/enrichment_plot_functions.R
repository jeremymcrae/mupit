...
label_genes <- function(enriched, p_values, num.tests) {    
    ...
    text(x=which(thresh.index), y=-log10(p_values[thresh.index]), 
        labels=enriched$hgnc[thresh.index], pos=3, cex=0.5)
}
...
#' @export
plot_values <- function(num.tests, enriched, p_values, title, color_index) {
    ...
    plot(-log10(p_values), col=color_index, pch=19, cex=0.75, 
        ...
        ylim=c(0, max(-log10(p_values), na.rm=TRUE) + 1))
    ...
    label_genes(enriched, p_values, num.tests)
}
